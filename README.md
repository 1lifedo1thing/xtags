...
![Xtags 实际使用截图：帖子旁显示“挑拨 0.68”意图标签](store/assets/screenshot-labels-original.png)

*作者提供的实际使用截图；红色箭头标出标签位置。图中分数来自旧提示词的一次判断，0.1.8 对同一帖子可能给出不同分数。标签为 AI 概率判断，可能存在误差。*
...
- **后台按 status ID 和正文指纹缓存、去重**。同一帖子正文变化后会重新判断，多个标签页可共享相同正文的结果；全局最多 3 个并发请求。缓存不保存正文原文。标签记录所属 status ID，虚拟列表复用节点时会检查并更新。
- **改阈值不重新请求**。缓存保存原始概率，每次渲染都按当前阈值组合标签，刷新页面也保持一致。支持 0–1 的阈值。
- **暂停会停止队列并取消在途请求**。清缓存、换模型或 key 后，旧请求结果不能写回；已经发送到服务端的请求无法保证撤回或免除费用。
- **失败恢复有边界**。每次请求超时 20 秒；网络故障、429 和 5xx 最多尝试 3 次，401 等错误不自动重试。正文发生变化、修正 key 或暂停后恢复可以重新尝试失败帖子。
...
- **没有开发者分析、遥测或分类服务器**。所选 API 服务会接收请求正文、作者账号和 IP 等连接信息；本地持久化缓存含帖子 ID、正文指纹、判断结果和时间戳，不保存正文原文。统计数字只在当前标签页内存中保留；费用为该页实际请求的估算，不含其他标签页和失败请求可能产生的费用。完整说明见[隐私政策发布稿](docs/privacy.zh-CN.html)。
