...
**提交前补一张真实时间线截图作为主图。** 在已验证的 X 页面实际运行扩展，截取至少一条文本帖子及它的真实标签，避免私人内容、API key、账户敏感信息。建议用自己公开发布且允许展示的测试帖子；不要手工编造概率。尺寸保持 1280 × 800，每种语言最多上传五张截图。两种语言的设置图分别放入对应语言 listing；繁体 listing 若使用简体界面图，应明确 UI 语言。
